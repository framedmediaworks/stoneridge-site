...
// Change NAV_ORDER to reorder the menu site-wide; pages listed here are
// arranged in this sequence on every page, so individual pages never drift.
const NAV_ORDER = ['index.html', 'homes.html', 'golf.html', 'amenities.html', 'blog.html', 'gallery.html', 'contact.html'];
document.querySelectorAll('nav.primary').forEach(nav => {
  if (!nav.querySelector('a[href="blog.html"]')) {
    const link = document.createElement('a');
    link.href = 'blog.html';
    link.textContent = 'News';
    if (window.location.pathname.split('/').pop() === 'blog.html') link.classList.add('active');
    nav.appendChild(link);
  }
  // re-appending an existing node moves it, so this both orders and de-duplicates
