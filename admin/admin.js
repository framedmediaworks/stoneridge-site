...
const COLLECTIONS = {
  ...
  gallery: { file: "content/gallery.json", key: "gallery", label: "Photo", list: [], sha: null, listEl: "galleryList", countEl: "countGallery" }
};
...
const POST_FIELDS = [
  ...
  { key: "spotlight", label: "Home Page Spotlight", type: "select", options: ["No", "Yes"], hint: "Set to Yes to feature this post in the Spotlight section on the home page. If more than one is set to Yes, the topmost one is used." },
...
const GALLERY_CATEGORIES = ["Lookout Ridge", "The Great Outdoors", "Community Events", "Life at StoneRidge", "Exteriors & Landscaping"];

const GALLERY_FIELDS = [
  { type: "section", label: "Photo" },
  { key: "image", label: "Photo", type: "photo" },
  { key: "caption", label: "Caption", type: "text", hint: "Describes the photo for screen readers and search engines." },
  { key: "category", label: "Category", type: "select", options: GALLERY_CATEGORIES }
];

const FIELDS = { homes: HOME_FIELDS, plans: PLAN_FIELDS, posts: POST_FIELDS, gallery: GALLERY_FIELDS };
...
function cardHTML(name, item, i, total) {
  ...
  if (name === "gallery") {
    ...
    return `<div class="admin-card">
      <div class="admin-card-photo ${src ? "" : "empty"}">
        <span class="admin-card-status">${esc(item.category || "Uncategorized")}</span>
        ${src ? `<img src="${esc(src)}" alt="">` : ""}
      </div>
      <div class="admin-card-body">
        <div class="admin-card-meta">${esc(item.caption || "No caption yet")}</div>
        <div class="admin-card-actions">
          <button class="btn btn-line" onclick="openEdit('gallery',${i})">Edit</button>
          ${reorderHTML(name, i, total)}
        </div>
      </div>
    </div>`;
...
$("addGalleryBtn").addEventListener("click", () => openEdit("gallery", null));
...
function openEdit(name, index) {
  ...
  if (name === "gallery" && !entry.category) entry.category = GALLERY_CATEGORIES[0];
  if (name === "posts") {
...
function updatePreview() {
  ...
  if (editing.name === "gallery") {
    stage.innerHTML = e.image
      ? `<div class="preview-post"><img src="${esc(resolveImg(e.image))}" alt=""><div class="pp-body"><div class="pp-date">${esc(e.category || "")}</div><p>${esc(e.caption || "No caption yet")}</p></div></div>`
      : `<p class="preview-note">Upload a photo and it will appear here.</p>`;
    return;
  }
